...
| Noctalia theme-template application gotchas | appearance | Found while trialing (then reverting) Wezterm as a Kitty replacement: a new `builtin_ids` entry needs a full daemon restart, not just `config-reload`; templates silently no-op if the app isn't installed; Wezterm's own color-render step never fired even when installed | [appearance/noctalia-template-application-gotchas.md](appearance/noctalia-template-application-gotchas.md) |
