...
| Mango border width matched, gradient approximated with a flat color | appearance | `borderpx = 2` to match Hyprland; Mango has no gradient-border support at all, so `focuscolor` set to the brighter of Hyprland's two active_border gradient stops | [appearance/mango-border-width-and-color.md](appearance/mango-border-width-and-color.md) |
...
| SUPER+B also launches the browser | keybindings | Matches Mango's browser key (B instead of W); added alongside the existing SUPER+W, not a replacement | [keybindings/browser-launch-super-b.md](keybindings/browser-launch-super-b.md) |
