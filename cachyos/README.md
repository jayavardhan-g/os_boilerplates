...
| CachyOS shell configs are package-owned — don't edit them | system | `/usr/share/cachyos-{fish,zsh}-config/` are root-owned pacman files that package updates silently overwrite (no `.pacnew`); personal aliases belong in `~/.config/fish/config.fish` / `~/.zshrc` *after* the `source` line — `pacman -Qkk` audits what's been hand-edited | [system/cachyos-shell-configs-are-package-owned.md](system/cachyos-shell-configs-are-package-owned.md) |
